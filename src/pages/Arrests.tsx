...
interface Criminal { _id: string; Criminal_Name: string; }
...
const emptyForm = { Arrest_Date: '', Criminal_ID: '', Case_ID: '', Charges: '' };
...
export default function Arrests() {
  ...
  useEffect(() => {
    api.get('/officer/criminals?limit=100').then(r => setCriminals(r.data.data)).catch(() => {});
  ...
  const openEdit   = (a: ArrestItem) => {
    setEditing(a);
    setForm({ Arrest_Date: a.Arrest_Date ? a.Arrest_Date.slice(0, 10) : '', Criminal_ID: a.Criminal_ID?._id || '', Case_ID: a.Case_ID?._id || '', Charges: a.Charges || '' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.delete(`/officer/arrests/${id}`); fetchData(); }
  ...
  return (
    <div className="p-8">
        ...
        {loading ? <div className="text-slate-400 p-6">Loading...</div> : (
          <table className="w-full">
            <thead className="bg-slate-900 border-b border-slate-700">
              <tr>
                {['Criminal', 'Arrest Date', 'Charges', 'Case Status', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-slate-700">
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-8 text-slate-400 text-center">No arrest records found.</td></tr>
              ) : items.map(a => (
                <tr key={a._id} className="hover:bg-slate-700/50 transition">
                  <td className="px-6 py-4 text-white font-medium">{a.Criminal_ID?.Criminal_Name || '—'}</td>
                  <td className="px-6 py-4 text-slate-400">{new Date(a.Arrest_Date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-slate-400 max-w-xs truncate">{a.Charges || '—'}</td>
                  ...
                  <td className="px-6 py-4 flex gap-3">
                    <button onClick={() => openEdit(a)} className="text-blue-400 hover:text-blue-300 text-sm font-medium">Edit</button>
                    <button onClick={() => handleDelete(a._id)} className="text-red-400 hover:text-red-300 text-sm font-medium">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showModal && (
        <Modal title={editing ? 'Edit Arrest' : 'Record Arrest'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            <Field label="Criminal *">
              <select value={form.Criminal_ID} onChange={e => setForm(f => ({ ...f, Criminal_ID: e.target.value }))} className={inputCls}>
                <option value="">Select criminal</option>
                {criminals.map(c => <option key={c._id} value={c._id}>{c.Criminal_Name}</option>)}
              </select>
            </Field>
            <Field label="Case *">
            ...
            <Field label="Arrest Date *">
              <input type="date" value={form.Arrest_Date} onChange={e => setForm(f => ({ ...f, Arrest_Date: e.target.value }))} className={inputCls} />
            </Field>
            <Field label="Charges">
              <textarea value={form.Charges} onChange={e => setForm(f => ({ ...f, Charges: e.target.value }))} className={`${inputCls} resize-none h-20`} placeholder="e.g. Robbery under IPC 392" />
            </Field>
