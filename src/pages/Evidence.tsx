...
export default function Evidence() {
  ...
  const openEdit   = (e: EvidenceItem) => {
    setEditing(e);
    setForm({ Evidence_Type: e.Evidence_Type, Description: e.Description || '', Case_ID: e.Case_ID?._id || '' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.delete(`/officer/evidence/${id}`); fetchData(); }
  ...
  return (
    <div className="p-8">
        ...
        {loading ? <div className="text-slate-400 p-6">Loading...</div> : (
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700">
              {items.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-8 text-slate-400 text-center">No evidence found.</td></tr>
              ) : items.map(e => (
                <tr key={e._id} className="hover:bg-slate-700/50 transition">
                  <td className="px-6 py-4 text-white font-medium">{e.Evidence_Type}</td>
                  <td className="px-6 py-4 text-slate-400 max-w-xs truncate">{e.Description || '—'}</td>
                  <td className="px-6 py-4 text-slate-400 max-w-xs truncate">{e.Case_ID?.Description || '—'}</td>
                  <td className="px-6 py-4 flex gap-3">
                    <button onClick={() => openEdit(e)} className="text-blue-400 hover:text-blue-300 text-sm font-medium">Edit</button>
                    <button onClick={() => handleDelete(e._id)} className="text-red-400 hover:text-red-300 text-sm font-medium">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showModal && (
        <Modal title={editing ? 'Edit Evidence' : 'Add Evidence'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            <Field label="Evidence Type *">
              <input value={form.Evidence_Type} onChange={e => setForm(f => ({ ...f, Evidence_Type: e.target.value }))} className={inputCls} placeholder="e.g. CCTV Footage, Weapon..." />
            </Field>
