...
export default function Victims() {
  ...
  const openEdit   = (v: Victim) => {
    setEditing(v);
    setForm({ Victim_Name: v.Victim_Name, Gender: v.Gender || '', Phone: v.Phone || '', Address: v.Address || '' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.delete(`/officer/victims/${id}`); fetchData(); }
  ...
  return (
    <div className="p-8">
            ...
            <tbody className="divide-y divide-slate-700">
              {items.length === 0 ? (
                ...
                <tr key={v._id} className="hover:bg-slate-700/50 transition">
                  <td className="px-6 py-4 text-white font-medium">{v.Victim_Name}</td>
                  <td className="px-6 py-4 text-slate-400">{v.Gender || '—'}</td>
                  <td className="px-6 py-4 text-slate-400">{v.Phone || '—'}</td>
                  <td className="px-6 py-4 text-slate-400 max-w-xs truncate">{v.Address || '—'}</td>
                  <td className="px-6 py-4 flex gap-3">
                    <button onClick={() => openEdit(v)} className="text-blue-400 hover:text-blue-300 text-sm font-medium">Edit</button>
                    <button onClick={() => handleDelete(v._id)} className="text-red-400 hover:text-red-300 text-sm font-medium">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showModal && (
        <Modal title={editing ? 'Edit Victim' : 'Add Victim'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            <Field label="Full Name *">
              <input value={form.Victim_Name} onChange={e => setForm(f => ({ ...f, Victim_Name: e.target.value }))} className={inputCls} />
            </Field>
            <Field label="Gender">
