...
interface Station { _id: string; Station_Name: string; }
interface Officer { _id: string; Officer_Name: string; }
...
const emptyForm = { Case_Date: '', Case_Status: 'Open', Description: '', Station_ID: '', Officer_ID: '' };
...
export default function Cases() {
  ...
  useEffect(() => {
    api.get('/officer/stations').then(r => setStations(r.data.data)).catch(() => {});
    api.get('/officer/officers?limit=100').then(r => setOfficers(r.data.data)).catch(() => {});
  }, []);
  ...
  return (
    <div className="p-8">
        ...
        {loading ? (
          ...
          <table className="w-full">
            <thead className="bg-slate-900 border-b border-slate-700">
              <tr>
                <th className="px-6 py-4 text-left text-slate-300">Description</th>
            ...
            <tbody className="divide-y divide-slate-700">
              {cases.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-8 text-slate-400 text-center">No cases found.</td></tr>
              ) : cases.map(c => (
                <tr key={c._id} className="hover:bg-slate-700/50 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusColor(c.Case_Status)}`}>{c.Case_Status}</span>
                  </td>
                  <td className="px-6 py-4 text-slate-400">{c.Officer_ID?.Officer_Name || '—'}</td>
                  <td className="px-6 py-4 text-slate-400">{c.Station_ID?.Station_Name || '—'}</td>
      ...
      {/* Modal */}
      {showModal && (
        <Modal title={editing ? 'Edit Case' : 'New Case'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            <Field label="Date">
              <input type="date" value={form.Case_Date} onChange={e => setForm(f => ({ ...f, Case_Date: e.target.value }))}
                className={inputCls} />
            </Field>
            <Field label="Status">
              <select value={form.Case_Status} onChange={e => setForm(f => ({ ...f, Case_Status: e.target.value }))} className={inputCls}>
                {STATUS_OPTIONS.map(s => <option key={s}>{s}</option>)}
              </select>
            </Field>
            ...
            <Field label="Station">
              <select value={form.Station_ID} onChange={e => setForm(f => ({ ...f, Station_ID: e.target.value }))} className={inputCls}>
                <option value="">Select station</option>
                {stations.map(s => <option key={s._id} value={s._id}>{s.Station_Name}</option>)}
              </select>
            </Field>
            <Field label="Officer">
              <select value={form.Officer_ID} onChange={e => setForm(f => ({ ...f, Officer_ID: e.target.value }))} className={inputCls}>
                <option value="">Select officer</option>
                {officers.map(o => <option key={o._id} value={o._id}>{o.Officer_Name}</option>)}
              </select>
            </Field>
