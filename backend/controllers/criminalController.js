...
const getCriminalStatus = async (req, res, next) => {
  ...
  try {
    ...
    if (!criminal)
      return res
        .status(404)
        .json({ success: false, message: "No data found for this criminal." });

    const arrests = await Arrest.find({ Criminal_ID: criminalId }).populate(
      "Case_ID",
      "Case_Status Case_Date Description",
    );
